...
# Load the CSV file
df_no_top = pd.read_csv('/data.lfpn/ibraun/Code/paper_volume_calculation/Idealized_model/Linear_Interpolation_Results/Linear_Interpolation_all.csv')  # Replace with your actual file path
df_all = pd.read_csv("/data.lfpn/ibraun/Code/paper_volume_calculation/Idealized_model/Linear_Interpolation_Results/Linear_Interpolation_no_top.csv")  # Replace with your actual file path
df_offset = pd.read_csv("/data.lfpn/ibraun/Code/paper_volume_calculation/Idealized_model/Linear_Interpolation_Results/Linear_Interpolation_offset.csv")  # Replace with your actual file path
...
scatter1 = ax.scatter(fil_df_no_top['distance'], fil_df_no_top['bp_vol'], 
                     color=colors['no_top'], s=50, alpha=0.5, linewidth=1, zorder=4)

# All Slices
line2 = ax.plot(fil_df_all['distance'], fil_df_all['bp_vol'], 
                color=colors['all_slices'], linewidth=2.5, 
                label="All Slices", alpha=0.6, zorder=3)
scatter2 = ax.scatter(fil_df_all['distance'], fil_df_all['bp_vol'], 
                     color=colors['all_slices'], s=50, alpha=0.5, linewidth=1, zorder=4)

# With Offset
line3 = ax.plot(fil_df_offset['distance'], fil_df_offset['bp_vol'], 
                color=colors['offset'], linewidth=2.5, 
                label="W Offset", alpha=0.6, zorder=3)
scatter3 = ax.scatter(fil_df_offset['distance'], fil_df_offset['bp_vol'], 
...
plt.savefig("/data.lfpn/ibraun/Code/paper_volume_calculation/Idealized_model/Graphics/linear_interpol_bp_vol.pdf", dpi=300, bbox_inches='tight', 
            facecolor='white', edgecolor='none', pad_inches=0.2)
plt.savefig("/data.lfpn/ibraun/Code/paper_volume_calculation/Idealized_model/Graphics/linear_interpol_bp_vol.svg", dpi=300, bbox_inches='tight', 
            facecolor='white', edgecolor='none', pad_inches=0.2)
# plt.show()